...
export class StandardEventFormatter implements LoggerEventFormatter {
	createEvent(
		{ time, level, name }: LogEventContext,
		messageOrError: string | Error | any,
		err: Error | any,
		...args: any[]
	) {
	...
	static formatArgs(
		...
		if (messageOrError instanceof Error) {
			...
			buff.push(
				`${err.message} ${JSON.stringify({ stacktrace: err.stack })}`
			);
...
export class LoggerWriterProxy implements LoggerWriter {
	...
	writeEvent(context: LogEventContext, ...args: any[]) {
...
export class LoggerFactory implements FactoryContainer {
	...
	setWriters(
		@Inject({
			matchInterface: INTERFACE_LOG_WRITER,
			matchCriteria: { min: 0 },
		})
