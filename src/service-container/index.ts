...
export class DependencyTracker {
	...
	interfaceAvailable(interfaze: string) {
		if (!this.waitingOnInterface[interfaze]) {
			dtLogger.debug(
				`interfaceAvailable: ${interfaze} -> NO_WAITING_DEPS`
			);
			return [];
		}
	...
	bindToInterface(
		...
		const min =
			depMeta.matchCriteria?.min === undefined
				? 1
				: depMeta.matchCriteria.min;
			...
			promises.push(
				this.initServiceFromRecord(rec)
					.then((inst) => {
						...
						const notifyServices = this.depTracker.serviceAvailable(
							rec.id
						);
						...
						this.wakeUpDependents(
							notifyServices.concat(notifyInterfaces)
						);
	...
	private async waitOnDependencies(
		...
		for (const dep of deps) {
			if (dep.startsWith('#')) {
				...
				this.depTracker.bindToInterface(interfaze, rec.id, {
					matchCriteria,
				});
			} else {
				...
				this.depTracker.bindToService(
					factoryId ? factoryId : dep,
					rec.id
				);
			}
		}
		...
		for (const _interface of notifyInterfaces) {
			...
			for (const svcId in this.interfaceSubscribers[_interface]) {
				...
				try {
					this.interfaceSubscribers[_interface][
						svcId
					].onAvailableInterface(_interface, [inst]);
				} catch (e) {
					this.logger.error(
			...
			if (miss == 0) {
				...
				for (const interfaze of rec.interfaces) {
					notify = notify.concat(
						depTrack.interfaceAvailable(interfaze)
					);
				}
