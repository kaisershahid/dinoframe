...
describe('module: morph', () => {
	...
	describe('MorphMarshaller polymorphic subclassing (agaist MorphPoly)', () => {
		...
		it('serializes to appropriate maps', () => {
			...
			expect(t.serialize(instA)).toEqual({
				name: 'instA-A',
				type: 'typeA',
				propA: '',
			});
		...
		it('fails deserializing unsupported discrinator value', () => {
			try {
				...
				expect((err as Error).message).toEqual(
					'MorphPoly: could not map type=undefined to a subclass: subclass is not a constructor'
				);
			}
			...
			try {
				...
				expect((err as Error).message).toEqual(
					'MorphPoly: could not map type=C to a subclass: subclass is not a constructor'
				);
			}
		});
	});
