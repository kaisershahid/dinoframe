import {
	...
	TransormerPropertyOverridesMap,
} from './types';
	...
	implements Morpher
{
	...
	private initProperties() {
		for (const propertyName in this.originalMeta.properties) {
			const { name, ...rest } =
				this.originalMeta.properties[propertyName][0];
			...
			try {
				...
				throw new MorphError(
					`${this.clazz.name}: could not map ${
						...
						source[this.clazz.___discriminatorCol]
					} to a subclass: ${(err as Error).message}`
				);
			}
		} else {
	...
	doDeserialize<T extends any = any>(
		inst: any,
		source: any,
		overrides: TransormerPropertyOverridesMap
	): T {
	...
	deserialize<T extends any = any>(
		...
		if (this.deserializeMethod) {
			inst[this.deserializeMethod as keyof T](source, this.manager);
			return inst;
		}
		...
		for (const name in this.propertyDefs) {
			...
			if (typeof def.type == 'function') {
				if (val instanceof Array) {
					...
					val = this.serializeNested(
						val,
						def.type as typeof Function
					);
				}
			}
	...
	update<T extends any = any>(
		inst: T,
		source: any,
		overrides?: TransormerPropertyOverridesMap
	) {
		this.doDeserialize(inst, { ...(inst as any), source }, overrides ?? {});
	}
}
		...
		if (val instanceof Array) {
			...
			if (err) {
				throw new FieldError(
					`one or more errors: ${JSON.stringify(errors)}`
				);
			}
		} else {
		...
		switch (def.type) {
			case 'boolean':
				if (typeof val != 'boolean') {
					throw new FieldError(
						`not a boolean: ${JSON.stringify(val)}`
					);
				}
				break;
			case 'string':
				if (typeof val != 'string') {
					throw new FieldError(
						`not a string: ${JSON.stringify(val)}`
					);
				}
				break;
			case 'number':
				if (typeof val != 'number') {
					throw new FieldError(
						`not a number: ${JSON.stringify(val)}`
					);
				}
				break;
			case 'enum':
...
export class BasicMorpherManager implements MorpherManager<MorphMarshaller> {
	...
	getByClassOrId(
		...
		const gid =
			typeof clazzOrId == 'string' ? clazzOrId : getGid(clazzOrId);
