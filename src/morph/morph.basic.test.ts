...
describe('module: morph', function () {
	describe('MorphMarshaller (against MorphBasic & MorphComplex)', () => {
		...
		class MorphBasic {
			...
			postDeserialize() {
				if (!this.title) {
					throw new ObjectError('MorphBasic', {
		...
		const getTransformer = () =>
			getMorpherById(MorphBasic) as MorphMarshaller;
		...
		it('deserializes and fails @Finalize', () => {
			try {
				...
				expect((err as Error).message).toEqual(
					'One or more errors for: MorphBasic'
				);
		...
		it(`deserializes and fails @Property.type=number ('a')`, () => {
			try {
				...
				expect((e as ObjectError).fieldErrors.aNumber.message).toEqual(
					'not a number: "a"'
				);
			}
		});
		...
		it(`deserializes and fails @Property.type=boolean ('a')`, () => {
			try {
				...
				expect((e as ObjectError).fieldErrors.aBoolean.message).toEqual(
					'not a boolean: "a"'
				);
			}
		});
		...
		it(`deserializes and fails @Property.type=string (true)`, () => {
			try {
				...
				expect(
					(e as ObjectError).fieldErrors.validatedString.message
				).toEqual('not a string: true');
			}
		});
		...
		it(`deserializes and fails @Property.type=enum ('e')`, () => {
			...
			try {
				...
				expect((e as ObjectError).fieldErrors.anEnum.message).toEqual(
		...
		it('deserializes with override', () => {
			try {
				...
				expect((e as ObjectError).fieldErrors.title.message).toEqual(
					'does not match "override"'
				);
			}
		});
	...
	describe('MorphMarshaller (manual serialization/deserialization)', () => {
		...
		it('propagates ObjectError on failed @Deserialize', () => {
			try {
				const inst = m?.deserialize<MorphManual>({
					...src,
					error: true,
				});
			} catch (e) {
				expect((e as ObjectError).fieldErrors).toEqual({
					...src,
					error: true,
				});
			}
	...
	describe('ValueFactory', () => {
		...
		[
			...
			({ type, success, val, enumValues, listType, expectedErr }) => {
				it(`${
					...
					expect(
						getMessage(val, { type, enumValues, listType })
					).toEqual(expectedErr);
				});
			}
		);
