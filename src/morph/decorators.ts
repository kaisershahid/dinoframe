    ...
    if (params.inherits?.discriminatorValue && !params.discriminator) {
      ...
      if (!parent) {
        throw new MorphError(
          `${target.name}: inherits.baseClass not specified`
        );
      }
      const dvalue = params.inherits.discriminatorValue;
      if (!dvalue) {
        throw new MorphError(
          `${target.name}: inherits.discriminatorValue not specified`
        );
      } else if (parent.___discriminatorMap[dvalue]) {
        throw new MorphError(
          `${target.name}: inherits.discriminatorValue '${dvalue}' already mapped to a subclass`
        );
      }
...
export const Property = (params: Partial<PropertyParams> = {}) => {
  return (target: any, name: string) => {
    if (
      ...
      throw new MorphError(
        `${target}: type set as 'enum' but enumValues is missing/empty`
      );
    }
    builder.pushProperty(
      target,
      name,
