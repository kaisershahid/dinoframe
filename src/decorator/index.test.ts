    ...
    desc.value = (...args: any[]) => {
      for (let i = 0; i < args.length; i++) {
        if (methDef.parameters[i]) {
          for (const rule of methDef.parameters[i] as ParameterMeta[]) {
...
describe("module: decorator", () => {
  describe("DecoratedClassBuilder", () => {
    ...
    it("processes Example", () => {
      ...
      expect(record.methods["method1"].parameters[0][0].matchRegex).toEqual(
        /hello/
      );
      expect(record.staticMethods["method2"]).not.toBeUndefined();
      expect(record.staticMethods["method2"].metadata[0].validate).toEqual(
        false
      );
      expect(record.staticMethods["method2"].metadata[0]._provider).toEqual(
        "test"
      );
      expect(record.staticMethods["method2"].metadata[0]._decorator).toEqual(
        "M1"
      );
