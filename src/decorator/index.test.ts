...
const DecoratorTestBundle2 = BundleDecoratorFactory(
	'dinoframe.decorator.example2.test'
);
...
describe('module: decorator', () => {
	describe('DecoratedClassBuilder', () => {
		...
		it('processes Example', () => {
			...
			expect(record.methods['method1'].metadata[0]._provider).toEqual(
				'test'
			);
			expect(record.methods['method1'].metadata[0]._decorator).toEqual(
				'M1'
			);
			...
			expect(
				record.staticMethods['method2'].metadata[0].validate
			).toEqual(false);
			expect(
				record.staticMethods['method2'].metadata[0]._provider
			).toEqual('test');
			expect(
				record.staticMethods['method2'].metadata[0]._decorator
			).toEqual('M1');
