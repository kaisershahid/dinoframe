 ...
 */
export type BoundHandlerPrecondition = (
  request: Request,
  response: Response
) => void;
...
export const extractParametersAsArgsFromRequest = (
  ...
  const sliceIndex =
    handlerConfig.metadata[0].type === HandlerConfigType.error ? 4 : 3;
      ...
      if (enumValues) {
        if (!enumValues.includes(arg)) {
          throw new RequestParamError(
            `${name}: got ${arg}; expected: ${JSON.stringify(enumValues)}`,
            param
          );
        }
      }
...
export class HttpDecoratorsBinder {
  ...
  bind(callback: BoundHandlerCallback) {
    const boundList: [
      ...
      HandlerConfig,
      DecoratedClass<ControllerConfig>
    ][] = [];
